 ...
 * Project Home Page: http://Flickr.com/
...
if ( !class_exists('Flickr') ) {
	...
	class Flickr {
		...
		function request ($command, $args = array(), $nocache = false)
		{
			...
			} elseif (!empty($_SESSION['Flickr_auth_token'])) {
				$args = array_merge($args, array("auth_token" => $_SESSION['Flickr_auth_token']));
			}
			ksort($args);
			$auth_sig = "";
		...
		function setProxy ($server, $port) {
			// Sets the proxy for all Flickr calls.
		...
		function getFriendlyGeodata ($lat, $lon) {
			 ...
			 */
			return unserialize(file_get_contents('http://Flickr.com/geodata/?format=php&lat=' . $lat . '&lon=' . $lon));
		}
...
//			if (empty($_SESSION['Flickr_auth_token']) && empty($this->token)) {
				if ( $remember_uri === true ) {
					$_SESSION['Flickr_auth_redirect'] = $_SERVER['REQUEST_URI'];
				} elseif ( $remember_uri !== false ) {
					$_SESSION['Flickr_auth_redirect'] = $remember_uri;
				}
...
//					header("Location: http://www.23hq.com/services/auth/?api_key=" . $this->api_key . "&perms=" . $perms . "&api_sig=". $api_sig);
					return "Location: http://www.23hq.com/services/auth/?api_key=" . $this->api_key . "&perms=" . $perms . "&api_sig=". $api_sig;
				} else {
...
//					unset($_SESSION['Flickr_auth_token']);
...
//			}
		...
		To use the Flickr::call method, pass a string containing the API method you want
		...
		*/
		...
		function auth_getToken ($frob) {
			...
			$_SESSION['Flickr_auth_token'] = $this->parsed_response['auth']['token'];
			return $this->parsed_response ? $this->parsed_response['auth'] : false;
		}
...
if ( !class_exists('Flickr_pager') ) {
	class Flickr_pager {
		var $Flickr, $per_page, $method, $args, $results, $global_Flickr;
		var $total = null, $page = 0, $pages = null, $photos, $_extra = null;
		
		
		function Flickr_pager($Flickr, $method = null, $args = null, $per_page = 30) {
			$this->per_page = $per_page;
			$this->method = $method;
			$this->args = $args;
			$this->set_Flickr($Flickr);
		}
		
		function set_Flickr($Flickr) {
			if ( is_a($Flickr, 'Flickr') ) {
				$this->Flickr = $Flickr;
				if ( $this->Flickr->cache ) {
		...
		function load($page) {
			...
			if ( $this->Flickr->cache ) {
				...
				if ( floor($min/500) == floor($max/500) ) {
					...
					$this->results = $this->Flickr->call($this->method, $this->args);
					if ( $this->results ) {
